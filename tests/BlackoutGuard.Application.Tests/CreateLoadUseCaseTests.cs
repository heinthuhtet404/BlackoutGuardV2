...
public class CreateLoadUseCaseTests
{
    ...
    public async Task Execute_ShouldFail_WhenRelayAddressConflicts()
    {
        ...
        Assert.Contains("Port 3", result.ErrorMessage);
    ...
    public async Task Execute_ShouldFail_WhenFacilityNotFound()
    {
        ...
        Assert.False(result.IsSuccess);
        Assert.Contains("Facility not found", result.ErrorMessage);
    }
    ...
    private sealed class FakeLoadRepository : ILoadRepository
    {
        ...
        public Task<LoadDto?> GetByRelayAddressAsync(Guid facilityId, int relayAddress, CancellationToken ct = default)
        {
            var match = Loads.FirstOrDefault(l => l.FacilityId == facilityId && l.RelayAddress == relayAddress);
            return Task.FromResult(match);
        }

        public Task<List<LoadDto>> GetP1LoadsAsync(Guid facilityId, CancellationToken ct = default)
        {
            var p1Loads = Loads.Where(l => l.FacilityId == facilityId && l.Priority == "P1" && l.IsActive).ToList();
