...
test("topology build: zones, loads, scoring, conflict", async ({ page }) => {
    ...
    // ✅ Wait for edit mode (URL changes to load ID or edit banner)
    await page.waitForURL(/\/topology\/loads\/[a-f0-9-]+/, { timeout: 15000 }).catch(async () => {
        // If URL doesn't change, wait for edit banner
        await expect(page.locator('[data-testid="edit-banner"]')).toBeVisible({ timeout: 10000 });
    });
    ...
    await page.click('[data-testid="save-button"]');
    await expect(page.getByText(/updated/i).first()).toBeVisible({ timeout: 5000 }).catch(() => {
        // If toast not found, just check that we're still in edit mode
        return expect(page.locator('[data-testid="edit-banner"]')).toBeVisible({ timeout: 5000 });
    });
