...
export function LoadForm({ loadId, initialValues, onSaved, onCreated }: LoadFormProps): ReactNode {
    ...
    // Criticality Risk States
    const [safetyRisk, setSafetyRisk] = useState(initialValues?.safetyRisk ?? 5);
    const [dataLossRisk, setDataLossRisk] = useState(initialValues?.dataLossRisk ?? 5);
    const [operationalRisk, setOperationalRisk] = useState(initialValues?.operationalRisk ?? 5);
    const [comfortRisk, setComfortRisk] = useState(initialValues?.comfortRisk ?? 5);

    const [relayError, setRelayError] = useState<string | null>(null);
    const [capacityError, setCapacityError] = useState<string | null>(null);
    const [generalError, setGeneralError] = useState<string | null>(null);
    const [saving, setSaving] = useState(false);
    const [manualPriority, setManualPriority] = useState<PriorityLevel | null>(null);
    ...
    return (
        <form className={styles.form} onSubmit={handleSubmit} noValidate>
            ...
            <CriticalityWizard
                loadId={loadId}
                onManualPriorityChange={setManualPriority}
                safety={safetyRisk}
                onSafetyChange={setSafetyRisk}
                dataLoss={dataLossRisk}
                onDataLossChange={setDataLossRisk}
                operational={operationalRisk}
                onOperationalChange={setOperationalRisk}
                comfort={comfortRisk}
                onComfortChange={setComfortRisk}
            />
