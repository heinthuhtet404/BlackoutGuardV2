...
function SimulatorPanelContent() {
  ...
  const handleFrequencyChange = (value: number) => {
    ...
    freqTimer.current = setTimeout(() => {
      void post("/simulator/telemetry", { frequency: value }).catch((err: unknown) => {
  ...
  const handleLoadChange = (value: number) => {
    ...
    loadTimer.current = setTimeout(() => {
      void post("/simulator/telemetry", { loadPercent: value }).catch((err: unknown) => {
  ...
  const handleGeneratorToggle = (value: boolean) => {
    ...
    void post("/simulator/telemetry", { generatorOn: value }).catch((err: unknown) => {
      ...
      <section className={styles.card}>
        ...
        <label className={styles.control}>
          <span className={styles.controlLabel}>
            Frequency: <strong data-testid="frequency-value">{frequency.toFixed(1)} Hz</strong>
          </span>
          <input
            type="range"
            min={FREQ_MIN}
          ...
          <span className={styles.rangeLabels}>
            <span>{FREQ_MIN.toFixed(1)} Hz</span>
            <span>{FREQ_MAX.toFixed(1)} Hz</span>
          </span>
        </label>

        <label className={styles.control}>
          <span className={styles.controlLabel}>
            Load: <strong data-testid="load-value">{loadPercent}%</strong>
          </span>
          <input
            type="range"
            min={LOAD_MIN}
